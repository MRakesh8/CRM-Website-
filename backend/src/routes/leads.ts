...
import { db, leadsTable, usersTable } from "@workspace/database";
...
router.post("/leads", validate(CreateLeadBody), async (req, res) => {
...
router.patch("/leads/:id", validate(UpdateLeadBody), async (req, res) => {
  const id = parseInt(req.params.id);
