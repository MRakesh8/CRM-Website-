...
import { db, paymentsTable, clientsTable, invoicesTable } from "@workspace/database";
...
router.post("/payments", validate(CreatePaymentBody), async (req, res) => {
...
router.patch("/payments/:id", validate(UpdatePaymentBody), async (req, res) => {
  const id = parseInt(req.params.id);
