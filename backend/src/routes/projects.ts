...
import { db, projectsTable, clientsTable } from "@workspace/database";
...
router.post("/projects", validate(CreateProjectBody), async (req, res) => {
...
router.patch("/projects/:id", validate(UpdateProjectBody), async (req, res) => {
  const id = parseInt(req.params.id);
