...
import { db, clientsTable } from "@workspace/database";
...
router.post("/clients", validate(CreateClientBody), async (req, res) => {
...
router.patch("/clients/:id", validate(UpdateClientBody), async (req, res) => {
  const id = parseInt(req.params.id);
