...
import { db, ticketsTable, clientsTable, usersTable } from "@workspace/database";
...
router.post("/tickets", validate(CreateTicketBody), async (req, res) => {
...
router.patch("/tickets/:id", validate(UpdateTicketBody), async (req, res) => {
  const id = parseInt(req.params.id);
