                ...
                {invoice.items && invoice.items.length > 0 ? (
                  invoice.items.map((item, i) => (
                    <tr key={i} className="hover:bg-muted/30 print:hover:bg-transparent">
                      ...
                      <td className="py-4 px-2 text-right text-muted-foreground">{formatCurrency(item.unitPrice)}</td>
                      <td className="py-4 px-2 text-right font-medium">{formatCurrency(item.quantity * item.unitPrice)}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start gap-8">
            ...
            <div className="w-full sm:w-1/3 space-y-3">
              ...
              <div className="flex justify-between items-end border-t-2 border-foreground pt-4 mt-2">
                ...
                <span className="text-3xl font-black text-primary print:text-black">{formatCurrency(invoice.total)}</span>
              </div>
            </div>
          </div>
