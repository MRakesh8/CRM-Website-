...
export function InvoiceDialog({ open, onOpenChange, invoice }: InvoiceDialogProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      notes: invoice?.notes || "",
      tax: invoice?.tax ? convertFromUSD(invoice.tax) : 0,
      discount: invoice?.discount ? convertFromUSD(invoice.discount) : 0,
      items: invoice?.items ? invoice.items.map((i: any) => ({ ...i, unitPrice: convertFromUSD(i.unitPrice) })) : [{ description: "", quantity: 1, unitPrice: 0 }],
    },
  });
  ...
  const onSubmit = (data: FormValues) => {
    const payload = {
      ...
      tax: convertToUSD(data.tax || 0),
      discount: convertToUSD(data.discount || 0),
      subtotal: convertToUSD(subtotal),
      total: convertToUSD(total),
      items: data.items.map(item => ({ 
        ...item, 
        unitPrice: convertToUSD(item.unitPrice),
        amount: convertToUSD(item.quantity * item.unitPrice) 
      }))
    };
              ...
              {fields.map((field, index) => (
                <div key={field.id} className="flex gap-4 items-start bg-muted/20 p-4 rounded-lg border">
                  ...
                  <div className="w-32 pt-8 text-right font-medium">
                    {formatCurrency(convertToUSD((watchedItems[index]?.quantity || 0) * (watchedItems[index]?.unitPrice || 0)))}
                  </div>
                  {fields.length > 1 && (
            ...
            <div className="flex justify-end gap-12 border-t pt-4">
              <div className="w-64 space-y-4">
                <div className="flex justify-between font-medium">
                  <span>Subtotal</span>
                  <span>{formatCurrency(convertToUSD(subtotal))}</span>
                </div>
                <FormField
                  control={form.control}
                  name="tax"
                  render={({ field }) => (
                    <FormItem className="flex justify-between items-center gap-4 space-y-0">
                      <FormLabel className="shrink-0">Tax ({currency})</FormLabel>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex justify-between items-center gap-4 space-y-0">
                      <FormLabel className="shrink-0">Discount ({currency})</FormLabel>
                      <FormControl><Input type="number" step="0.01" min={0} className="w-32 text-right" {...field} /></FormControl>
                    </FormItem>
                  )}
                />
                <div className="flex justify-between font-bold text-lg border-t pt-2">
                  <span>Total</span>
                  <span>{formatCurrency(convertToUSD(Math.max(0, total)))}</span>
                </div>
              </div>
            </div>
